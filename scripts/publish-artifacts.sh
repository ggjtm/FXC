...
# Stage INSIDE the docroot, not /tmp. Two reasons: the tarballs are ~180 MB and the master's root
# fs is small, and — the real one — the final `mv` below is only atomic within a filesystem. With
# STAGE on / and the docroot on its own volume, every "rename" was a cross-device copy, so a reader
# could see a half-written tar (fxc/docs/PROBLEMS.md P25). Inside the docroot rather than beside it
# because the docroot is what the build user owns; the dot prefix keeps the staging dir out of
# lighttpd's directory listing while it exists.
STAGE="$(mktemp -d -p "${DOCROOT}" .publish-XXXXXX)"
