...
**Runtime state is not a build product**, and `./gradlew clean` does not touch it: the cold archives
live in the MariaDB volume and each GridGain node leaves a work directory under `$TMPDIR`
(`fxc-{exchange,broker,pub}-ignite`). Both survive a rebuild and accumulate across demo runs.
`scripts/reset.sh` is the eraser — see README "Starting from a clean market". Run it with the demo
stopped; `--dry-run` shows what would go.
