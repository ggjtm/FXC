...
class CatalogTest(unittest.TestCase):
    #: Exactly InstrumentCatalog.LISTINGS, symbol -> reference price. Pinned here because the two
    #: catalogs are hand-mirrored across languages: without this, a price that drifts between the
    #: Java seeding and the Python harness is invisible until a book prices itself wrongly.
    EXPECTED_EQUITIES = {
        "ARVX": "18.25", "BLTN": "9.60", "CRVN": "46.75", "DYNL": "132.40", "ELXR": "74.15",
        "FRTH": "31.05", "GRVT": "208.60", "HLYN": "22.90", "IVRN": "63.30", "JNTR": "15.45",
        "KLSO": "38.70", "LUMR": "87.25", "MRDN": "26.15", "NVSK": "154.80", "ORBN": "96.50",
        "PLTH": "11.35", "QRVN": "41.60", "RDSN": "58.95", "STLR": "19.80", "TRQL": "33.20",
        "UPLN": "112.05", "VNTA": "68.40", "WSTB": "14.70", "XNTH": "245.30", "YRRA": "29.55",
    }

    def test_mirrors_the_java_catalog(self):
        # The tradable universe is exactly the 25 listed equities.
        self.assertEqual(set(self.EXPECTED_EQUITIES), set(instruments.EQUITIES))

    def test_reference_prices_mirror_the_java_listings(self):
        for symbol, price in self.EXPECTED_EQUITIES.items():
            self.assertEqual(Decimal(price), instruments.find(symbol).reference_price, symbol)

    def test_fx_pairs_are_known_but_not_listed(self):
        # De-listed (fxc/docs/PROBLEMS.md P23) but still in CATALOG: the OFX "FX:" wire branch and
        # its golden fixture are live code that must keep resolving tick/lot.
        for pair in ("EUR/USD", "GBP/USD", "USD/JPY", "AUD/USD"):
            self.assertIn(pair, instruments.CATALOG)
            self.assertNotIn(pair, instruments.EQUITIES)
    ...
    def test_tick_and_lot_sizes(self):
        ...
        self.assertEqual(Decimal("0.01"), instruments.find("ARVX").tick_size)
        self.assertEqual(Decimal("1"), instruments.find("ARVX").lot_size)
    ...
    def test_fx_flag_and_quote_currency(self):
        ...
        self.assertFalse(instruments.find("ARVX").is_fx)
        self.assertEqual("USD", instruments.find("ARVX").quote_currency)

    def test_unknown_symbol_names_the_alternatives(self):
        with self.assertRaises(ValueError) as caught:
            instruments.find("NOPE")
        self.assertIn("ARVX", str(caught.exception))


class SnapToTickTest(unittest.TestCase):
    def test_equity_snaps_to_a_cent_and_carries_the_tick_scale(self):
        self.assertEqual(Decimal("42.10"), instruments.snap_to_tick("ARVX", "42.104"))
        self.assertEqual(Decimal("42.11"), instruments.snap_to_tick("ARVX", "42.105"))  # HALF_UP
        # The result carries the tick's scale, so the emitted decimal has the expected places.
        self.assertEqual(2, -instruments.snap_to_tick("ARVX", "42.1").as_tuple().exponent)
    ...
    def test_snapped_prices_survive_the_ofx_number_format(self):
        ...
        for symbol, raw in (("ARVX", "42.104"), ("EUR/USD", "1.084204"), ("USD/JPY", "156.2504")):
...
class SnapToLotTest(unittest.TestCase):
    def test_equity_lot_is_one(self):
        self.assertEqual(Decimal("7"), instruments.snap_to_lot("ARVX", "7.9"))
    ...
    def test_zero_and_negative(self):
        self.assertEqual(Decimal("0"), instruments.snap_to_lot("ARVX", "0"))
        self.assertEqual(Decimal("-1"), instruments.snap_to_lot("ARVX", "-0.5"))
