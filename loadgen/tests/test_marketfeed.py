...
class VolumeByPriceTest(unittest.TestCase):
    ...
    def test_missing_volume_by_price_is_empty_not_fatal(self):
        self.assertEqual({}, marketfeed.volume_by_price({"symbol": "ACME", "candles": []}))
...
class CandlesUrlTest(unittest.TestCase):
    def test_asks_for_a_bounded_window(self):
        url = marketfeed.candles_url("http://exchange:8090", "ACME", window_ms=60_000,
                                     now_ms=1_753_800_060_000)
        self.assertEqual(
            "http://exchange:8090/api/candles?symbol=ACME&granularity=1m&start=1753800000000", url
        )

    def test_trailing_slash_does_not_double_up(self):
        url = marketfeed.candles_url("http://exchange:8090/", "ACME", now_ms=0)
    ...
    def test_start_never_goes_negative(self):
        url = marketfeed.candles_url("http://x", "ACME", window_ms=10_000, now_ms=5_000)
...
class FetchTest(unittest.TestCase):
    def test_fetches_and_parses_through_an_injected_fetcher(self):
        ...
        histogram = marketfeed.fetch_volume_by_price("http://exchange:8090", "ACME", fetcher=fetcher)
        self.assertEqual(3, len(histogram))
        self.assertIn("symbol=ACME", seen["url"])
    ...
    def test_unusable_content_is_empty_rather_than_an_error(self):
        histogram = marketfeed.fetch_volume_by_price(
            "http://exchange:8090", "ACME", fetcher=lambda url, timeout: "<html>nope</html>"
        )
    ...
    def test_transport_failures_are_reported_not_swallowed(self):
        ...
        with self.assertRaises(marketfeed.MarketFeedError) as caught:
            marketfeed.fetch_volume_by_price("http://exchange:8090", "ACME", fetcher=broken)
...
class MarketViewIntegrationTest(unittest.TestCase):
    def test_feed_histogram_makes_bookfish_ready(self):
        ...
        market.set_last_sale("ACME", Decimal("42.20"))
        market.load_traded_volume("ACME", marketfeed.volume_by_price(CANDLE_RESPONSE))
    ...
    def test_a_later_read_replaces_rather_than_accumulates(self):
        market = strategies.MarketView()
        market.load_traded_volume("ACME", {Decimal("42.1"): Decimal("60")})
        market.load_traded_volume("ACME", {Decimal("42.1"): Decimal("70")})
        self.assertEqual({Decimal("42.1"): Decimal("70")}, market.traded_volume["ACME"])

    def test_an_empty_read_leaves_the_previous_view_alone(self):
        market = strategies.MarketView()
        market.load_traded_volume("ACME", {Decimal("42.1"): Decimal("60")})
        market.load_traded_volume("ACME", {})
        self.assertEqual({Decimal("42.1"): Decimal("60")}, market.traded_volume["ACME"])
